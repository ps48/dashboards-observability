...
export function ServiceMap({
  ...
  const [selectableValue, setSelectableValue] = useState<EuiSuperSelectOption<any>[]>([]);

  const onChangeSelectable = (value: React.SetStateAction<EuiSuperSelectOption<any>[]>) => {
    ...
    if (selectableValue.length === 0 && value.length !== 0) {
      includeMetricsCallback && includeMetricsCallback();
    }
  ...
  const metricOptions: EuiSuperSelectOption<any>[] = [
    {
      value: 'latency',
