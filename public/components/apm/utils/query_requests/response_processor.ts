...
export function transposeDataFrame(dataFrame: PPLDataFrame): Array<Record<string, any>> {
  ...
  if (dataFrame?.jsonData && Array.isArray(dataFrame.jsonData)) {
    console.log('Using jsonData from PPL facet:', dataFrame.jsonData.length, 'rows');
...
export function transformListServicesResponse(pplResponse: PPLDataFrame): ListServicesResponse {
  ...
  console.log(`[DEBUG] transformListServicesResponse: Transposed ${rows.length} rows`);
  if (rows.length > 0) {
    console.log(
      `[DEBUG] transformListServicesResponse: Sample row[0]:`,
  ...
  rows.forEach((row, index) => {
    ...
    if (!serviceName || !environmentType) {
      console.log(`[DEBUG] Row ${index}: SKIPPED - missing serviceName or EnvironmentType`);
    ...
    if (!serviceMap.has(key)) {
      ...
      console.log(`[DEBUG] Row ${index}: DUPLICATE - key ${key} already exists`);
    }
  });

  console.log(`[DEBUG] transformListServicesResponse: serviceMap size = ${serviceMap.size}`);
  ...
  if (ServiceSummaries.length > 0) {
    console.log(
      `[DEBUG] transformListServicesResponse: Sample ServiceSummary[0]:`,
      JSON.stringify(ServiceSummaries[0], null, 2)
    );
  }
...
export function transformGetServiceResponse(pplResponse: PPLDataFrame): any {
  ...
  console.log(`[DEBUG] transformGetServiceResponse: Received ${rows.length} rows`);
  if (rows.length > 0) {
    console.log(
      `[DEBUG] transformGetServiceResponse: Sample row[0]:`,
      JSON.stringify(rows[0], null, 2)
    );
  }
  ...
  console.log(
    `[DEBUG] transformGetServiceResponse: serviceName=${serviceName}, environment=${environmentType}`
  );

  if (!serviceName || !environmentType) {
    console.log(`[DEBUG] transformGetServiceResponse: Missing required fields`);
    return {
      Service: null,
      StartTime,
      EndTime,
  ...
  console.log(
    `[DEBUG] transformGetServiceResponse: Returning service with ${attributeMaps.length} attribute maps`
  );
...
export function transformListServiceOperationsResponse(pplResponse: PPLDataFrame): any {
  ...
  console.log(`[DEBUG] transformListServiceOperationsResponse: Received ${rows.length} rows`);
  if (rows.length > 0) {
    console.log(
      `[DEBUG] transformListServiceOperationsResponse: Sample row[0]:`,
      JSON.stringify(rows[0], null, 2)
    );
  }
  ...
  console.log(
    `[DEBUG] transformListServiceOperationsResponse: Found ${operations.length} unique operations`
  );
...
export function transformListServiceDependenciesResponse(pplResponse: PPLDataFrame): any {
  ...
  console.log(`[DEBUG] transformListServiceDependenciesResponse: Received ${rows.length} rows`);
  if (rows.length > 0) {
    console.log(
      `[DEBUG] transformListServiceDependenciesResponse: Sample row[0]:`,
      JSON.stringify(rows[0], null, 2)
    );
  }
...
export function transformGetServiceMapResponse(pplResponse: PPLDataFrame): any {
  ...
  console.log(`[DEBUG] transformGetServiceMapResponse: Received ${rows.length} rows`);
  if (rows.length > 0) {
    console.log(
      `[DEBUG] transformGetServiceMapResponse: Sample row[0]:`,
      JSON.stringify(rows[0], null, 2)
    );
  }
  ...
  rows.forEach((row, index) => {
    ...
    console.log(
      `[DEBUG] Row ${index}: serviceName=${serviceName}, environment=${environmentType}, platformType=${platformType}`
    );
    ...
    if (serviceName && environmentType) {
      ...
      if (!nodeMap.has(nodeKey)) {
        ...
        console.log(`[DEBUG] Added service node: ${nodeKey}`);
      } else {
        ...
        if (serviceGroupByAttributes && Object.keys(serviceGroupByAttributes).length > 0) {
          ...
          allGroupByAttributes.push(serviceGroupByAttributes);
          console.log(`[DEBUG] Merged groupByAttributes for existing service node: ${nodeKey}`);
        }
      }
    }
    ...
    if (remoteServiceName && remoteEnvironment) {
      ...
      if (!nodeMap.has(depNodeKey)) {
        ...
        console.log(`[DEBUG] Added remote service node: ${depNodeKey}`);
      } else {
        // Node already exists, merge new groupByAttributes without overwriting existing keys
        ...
        if (
          ...
          allGroupByAttributes.push(remoteServiceGroupByAttributes);
          console.log(
            `[DEBUG] Merged groupByAttributes for existing remote service node: ${depNodeKey}`
          );
        }
      }
    }
  ...
  console.log(`[DEBUG] transformGetServiceMapResponse: Created ${nodeMap.size} unique nodes`);
  ...
  const edges = rows
    .map((row, index) => {
      ...
      console.log(`[DEBUG] Created edge: ${serviceName} -> ${remoteServiceName}`);
  ...
  console.log(`[DEBUG] transformGetServiceMapResponse: Created ${edges.length} edges`);
  ...
  console.log(
    `[DEBUG] transformGetServiceMapResponse: Returning ${nodes.length} nodes and ${edges.length} edges`
  );
  console.log(
    `[DEBUG] transformGetServiceMapResponse: AvailableGroupByAttributes has ${
      Object.keys(availableGroupByAttributes).length
    } attribute paths`
  );
...
export function transformListServicesStatsResponse(pplResponse: PPLDataFrame): any[] {
  ...
  console.log(`[DEBUG] transformListServicesStatsResponse: Transposed ${rows.length} rows`);
  if (rows.length > 0) {
    console.log(
      `[DEBUG] transformListServicesStatsResponse: Sample row[0]:`,
      JSON.stringify(rows[0], null, 2)
    );
  }
  ...
  const services = rows
    .map((row) => {
      ...
      console.log(`[DEBUG] Processing service: name=${serviceName}, environment=${environment}`);

      if (!serviceName || !environment) {
        console.log(`[DEBUG] SKIPPED - missing serviceName or environment`);
        return null;
      }
  ...
  console.log(`[DEBUG] transformListServicesStatsResponse: Returning ${services.length} services`);
